...
function parseSafeDateOrNull(val) {
  if (!val) return null;
  if (val instanceof Date) {
    ...
    if (model === "company_profiles") {
      ...
      await prisma.quotations.upsert({
        ...
        update: {
          ...
          date: data.date ? parseSafeDate(data.date) : /* @__PURE__ */ new Date(),
        ...
        create: {
          ...
          customerSignedAt: parseSafeDateOrNull(data.customerSignedAt),
          revisionNumber: data.revisionNumber,
          createdAt: data.createdAt ? parseSafeDate(data.createdAt) : /* @__PURE__ */ new Date(),
          updatedAt: /* @__PURE__ */ new Date()
        }
      });
      ...
      await prisma.inventoryItems.upsert({
        ...
        update: {
          ...
          lastUpdated: data.lastUpdated ? parseSafeDate(data.lastUpdated) : /* @__PURE__ */ new Date()
        },
        create: {
          id: data.id,
          sku: cleanSku,
        ...
        for (const log of data.logs) {
          await prisma.inventoryLogs.create({
            data: {
              ...
              date: log.date ? parseSafeDate(log.date) : /* @__PURE__ */ new Date(),
              type: log.type || "IN",
